...
function expandContextTerm(
  ...
  if (definition && !seen.has(value)) {
    ...
    return expandContextTerm(definition.id, context, true, nextSeen)
  }
...
function applyLicenseContext(value: unknown, context: LicenseContext) {
  ...
  if (typeof value === 'string') {
    if (/^https?:\/\/w3id\.org\/ro\/crate\/1\.[12]\/context\/?$/.test(value)) {
      // The bundled 1.1 and 1.2 contexts map license without IRI coercion.
...
function licenseRow(entry: LicenseEntry, graph: Array<Record<string, unknown>>): LicenseRow {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    if (typeof value === 'string' && coercion) {
      ...
      const kind = entity || !/^[A-Za-z][A-Za-z0-9+.-]*:/.test(id)
        ? 'Internal reference'
        : 'External IRI'
      return {
        kind,
  ...
  const kind = entity || embedded
    ? 'Internal reference'
    : /^[A-Za-z][A-Za-z0-9+.-]*:/.test(expandedId)
      ? 'External IRI'
      : 'Internal reference'
  return {
    kind,
...
const licenseRows = computed<LicenseRow[]>(() => {
  ...
  const context = licenseContext(currentCrate.value, root)
  const entries = licenseEntries(root, context)
  if (!entries.length && current.value?.license) {
