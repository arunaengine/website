...
import { useJobs, useJobsList } from '@/composables/useJobs'
...
import { ChevronRight, ListTodo, LogIn, RefreshCw } from '@lucide/vue'
...
// closes it, ComputeView's taskId precedent).
const openJobId = computed(() => (route.params.jobId ? String(route.params.jobId) : ''))
...
function startSignIn() {
  void signIn({ redirectTo: '/app/jobs' })
}
...
onMounted(() => {
  // currentUser bootstraps after mount on a hard load/deep link; load once it
  // is present, otherwise the watch below picks up the late sign-in.
  if (jobsEnabled.value && currentUser.value) void list.load()
})
watch(currentUser, (u, prev) => {
  if (u && !prev && jobsEnabled.value && listState.value === 'idle') void list.load()
})
</script>

<template>
  <div>
    <PageHeader title="Jobs" description="Monitor durable background jobs owned by your account." />

    <!-- Gate 1: feature disabled — no API call is ever issued in this state. -->
    <div v-if="!jobsEnabled" class="container max-w-[1100px] py-8">
      <EmptyState
        title="Jobs are not enabled"
        description="Enable the jobs feature flag in portal-config.json once this realm's nodes serve the durable jobs API."
      >
        <template #icon><ListTodo class="h-7 w-7" /></template>
      </EmptyState>
    </div>

    <!-- Gate 2: not signed in — the job list is owner-scoped. -->
    <div v-else-if="!currentUser" class="container max-w-[1100px] py-8">
      <section class="surface mx-auto max-w-xl p-8 text-center">
        <ListTodo class="mx-auto h-8 w-8 text-muted-foreground/70" />
        <h2 class="mt-3 font-display text-base font-semibold text-aruna-navy">Sign in to monitor jobs</h2>
        <p class="mt-1.5 text-sm text-muted-foreground">The job list shows only your own durable jobs.</p>
    ...
    <div v-else class="container max-w-[1100px] space-y-5 py-8">
      <div class="flex flex-wrap items-center gap-2">
        <Select v-model="stateFilterModel" :options="stateOptions" class="h-8 w-44 text-xs" />
      ...
      <EmptyState
        ...
        :description="stateFilter ? 'Try a different state filter.' : 'Durable background jobs submitted by your account appear here.'"
      >
        <template #icon><ListTodo class="h-7 w-7" /></template>
      </EmptyState>
