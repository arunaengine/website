...
// core/src/structs/job.rs on branch feat/job-framework. NOT served by deployed
// backends yet, so all consumers gate on featureEnabled('jobs') (ships off)
// and treat 404/405 on the LIST route as "backend does not serve the jobs API
// yet". The surface is owner-scoped and rejects path-restricted (delegated)
// tokens with 403.
...
export const JOB_STATE_ORDER: JobState[] = ['queued', 'claimed', 'running', 'succeeded', 'failed', 'cancelled']
