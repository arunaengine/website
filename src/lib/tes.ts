...
// ASSUMED API — NO aruna backend serves TES yet (api/src/routes/mod.rs has no
// tes router; the only GA4GH surface is DRS in routes/drs.rs). All consumers
// gate on featureEnabled('tes'); the flag ships off. Types below mirror the
// GA4GH Task Execution Service v1.1 OpenAPI schema VERBATIM (snake_case,
// output-only fields optional) so flipping to a real backend needs zero type
// changes. Assumed mount mirrors routes/drs.rs: {apiBase}/ga4gh/tes/v1/... .
...
export interface TesOutput {
  ...
  url: string
  path: string
  // TES 1.1: required when `path` contains wildcards.
  path_prefix?: string
  type?: TesFileType
}
...
export interface TesResources {
  ...
  disk_gb?: number
  zones?: string[]
  backend_parameters?: Record<string, string> // TES 1.1
  backend_parameters_strict?: boolean // TES 1.1
}
...
export interface TesTask {
  ...
  executors: TesExecutor[] // required, min one
  volumes?: string[]
...
function pruneInput(input: TesInput): TesInput {
  ...
  if (input.content !== undefined && input.content !== '') out.content = input.content
  if (input.streamable) out.streamable = true
  return out
}
...
function pruneExecutor(executor: TesExecutor): TesExecutor {
  ...
  const stdin = trimmed(executor.stdin)
  if (stdin) out.stdin = stdin
  const stdout = trimmed(executor.stdout)
  if (stdout) out.stdout = stdout
  const stderr = trimmed(executor.stderr)
  if (stderr) out.stderr = stderr
  const env = pruneRecord(executor.env)
  if (env) out.env = env
  if (executor.ignore_error) out.ignore_error = true
  return out
}
...
function pruneResources(resources: TesResources | undefined): TesResources | undefined {
  ...
  if (resources.zones?.length) out.zones = [...resources.zones]
  const backend = pruneRecord(resources.backend_parameters)
  if (backend) out.backend_parameters = backend
  if (resources.backend_parameters_strict) out.backend_parameters_strict = true
  return Object.keys(out).length ? out : undefined
}
...
export function pruneTesTask(task: TesTask): TesTask {
  const out: TesTask = { executors: task.executors.map(pruneExecutor) }
