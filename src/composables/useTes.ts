...
// Compute orchestration via GA4GH TES (aruna#290). NO aruna backend serves TES
// yet — api/src/routes/mod.rs has no tes router; the only GA4GH surface is DRS.
// Every path here is an ASSUMED endpoint gated behind the `tes` feature flag, so
// with the default config (features: {}) this module is inert: no HTTP call can
// fire because every function starts with assertEnabled(). Assumed mount mirrors
// routes/drs.rs: {apiBase}/ga4gh/tes/v1/... .
...
// "Flag on, backend without aruna#290 yet" — consumers render the honest
// not-served-yet panel instead of a raw error (pattern from #269).
