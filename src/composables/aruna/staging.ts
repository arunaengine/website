...
// callers must show a running state. The axum route is literally "/data/staging".
export async function stageBlob(input: StageBlobSubmission): Promise<StageBlobResponse> {
  return request<StageBlobResponse>('/data/staging', { method: 'POST', body: JSON.stringify(input) })
}
...
export async function stageBatch(input: StagingBatchRequest): Promise<StagingBatchResponse> {
  return request<StagingBatchResponse>('/data/staging/batch', { method: 'POST', body: JSON.stringify(input) })
}

export async function listStagingJobs(cursor?: string, limit = 50): Promise<ListStagingJobsResponse> {
  return request<ListStagingJobsResponse>('/data/staging/jobs', { query: { limit, cursor } })
}

export async function createStagingJob(input: StagingBatchRequest): Promise<CreateStagingJobResponse> {
  return request<CreateStagingJobResponse>('/data/staging/jobs', {
    method: 'POST',
...
export async function getStagingJob(jobId: string): Promise<StagingJob> {
  return request<StagingJob>(`/data/staging/jobs/${encodeURIComponent(jobId)}`)
}
...
export async function listStagingReferences(
  ...
  do {
    const page = await request<StagingReferencesResponse>('/data/staging/references', {
      signal,
