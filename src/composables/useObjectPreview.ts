...
export function useObjectPreview() {
  ...
  async function fallbackDownload(target: PreviewTarget, cap: number, actual: number) {
    ...
    try {
      directUrl.value = await s3.downloadUrl(target.bucket, target.key)
      status.value = 'ready'
    } catch (err) {
  ...
  async function load(target: PreviewTarget) {
    ...
    try {
      ...
      if (classified.kind === 'image') {
        const blob = await s3.getObjectBlob(target.bucket, target.key)
        if (blob.size > cap) {
        ...
        const content = await s3.getObjectText(target.bucket, target.key)
