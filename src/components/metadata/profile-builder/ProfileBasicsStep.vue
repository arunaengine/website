  ...
  <section class="space-y-4">
    ...
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      <div>
        ...
        <Select v-model="builder.groupId" :options="builder.groupOptions" class="mt-1" placeholder="Choose a group" />
      </div>
      <div>
        <label class="text-xs font-medium text-foreground">Name</label>
        <Input v-model="builder.name" class="mt-1" placeholder="Proteomics Dataset Profile" />
      </div>
      <div>
      ...
      <div>
        ...
        <Input v-model="builder.version" class="mt-1" placeholder="0.1.0" />
      </div>
      <div>
        <label class="text-xs font-medium text-foreground">Date published</label>
        <Input v-model="builder.datePublished" type="date" class="mt-1" />
      </div>
      <div>
        <label class="text-xs font-medium text-foreground">License URL</label>
        <Input v-model="builder.license" class="mt-1" />
      </div>
    </div>

    <div>
      <label class="text-xs font-medium text-foreground">Description</label>
      <Textarea v-model="builder.description" class="mt-1" rows="2" placeholder="What this profile is for and who should use it." />
    </div>
