...
import type { TesFileType, TesInput } from '@/lib/tes'
...
function addEntry(entry: { url: string; path: string; type: TesFileType; name?: string }) {
  const input: TesInput = { path: entry.path, url: entry.url, type: entry.type }
    ...
    <div v-else class="space-y-2">
      <div v-for="(input, i) in modelValue" :key="i" class="surface flex flex-wrap items-center gap-3 p-3">
        <div class="min-w-0 flex-1">
          <div class="truncate font-mono text-[11px] text-foreground" :title="input.url">{{ input.url ? truncateMiddle(input.url, 20, 16) : '(inline content)' }}</div>
        </div>
        <Badge variant="outline">{{ input.type || 'FILE' }}</Badge>
