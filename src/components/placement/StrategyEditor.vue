...
import type { RealmPlacementStrategy } from '@/lib/api'
...
const props = defineProps<{
  modelValue: RealmPlacementStrategy
...
function update(patch: Partial<RealmPlacementStrategy>) {
...
const shardCount = computed<string | number>({
  get: () => props.modelValue.shard_count,
  set: (v) => update({ shard_count: Math.max(1, Math.floor(Number(v) || 1)) }),
})
...
function ruleLabel(rule: RealmPlacementStrategy['affinity'][number]): string {
  const target = `${rule.key}=${rule.value}`
  return rule.effect.kind === 'multiply' ? `${target} → ×${rule.effect.permille}‰` : `${target} → filter`
}
</script>
    ...
    <div class="grid gap-3 sm:grid-cols-2">
      ...
      <div>
        ...
        <Input
          ...
          :title="shardCountLocked ? 'The run family shard count is frozen.' : undefined"
        />
        <p class="mt-1 text-[11px] text-muted-foreground">
          {{ shardCountLocked ? 'Frozen for the run family.' : 'Power of two, at most 4096.' }}
        </p>
      </div>
    </div>
